...
import { createModel, trimPrompt, getPPQApiKey, generateWithPPQ } from './ai/providers';
...
export async function writeFinalReport({
  ...
  // Prepare the prompt
  const finalPrompt = `Given the following prompt from the user, write a final report on the topic using the learnings from research and format it in proper Markdown. Use Markdown syntax (headings, lists, horizontal rules, etc.) to structure the document. Aim for a detailed report of at least 3 pages.\n\n<prompt>${prompt}</prompt>\n\nHere are all the learnings from previous research:\n\n<learnings>\n${learningsString}\n</learnings>`;
